...
public class WritingCoachController : Controller
{
    ...
    public WritingCoachController(AppDbContext context, IAIService aiService, WritingCoachFallbackService fallbackService)
    {
        _context = context;
    ...
    public async Task<IActionResult> Generate(WritingCoachViewModel model)
    {
        ...
        if (setting != null && setting.IsEnabled && !string.IsNullOrWhiteSpace(setting.ApiKey))
        {
            ...
            {
                response = _fallbackService.GenerateResponse(model.Mode, model.Topic, model.EssayType, model.ThesisStatement, model.UserInput, model.Language);
            }
        }
        else
        {
        ...
        TempData[usedAi ? "SuccessMessage" : "WarningMessage"] = usedAi
            ? "Writing coach response generated by AI."
            : "AI unavailable. A rule-based writing guide was generated.";
    ...
    public async Task<IActionResult> Delete(int id)
    {
        ...
        TempData["SuccessMessage"] = session.Language == "vi"
            ? "Đã xóa phiên hỗ trợ viết thành công."
            : "Writing coach session deleted successfully.";
    ...
    private void ValidateCoachInput(WritingCoachViewModel model)
    {
        ...
        {
            ModelState.AddModelError(nameof(model.Topic), "Please enter a topic or additional instructions.");
        }

        if (model.Mode == "ThesisImprover" && string.IsNullOrWhiteSpace(model.ThesisStatement))
        {
            ModelState.AddModelError(nameof(model.ThesisStatement), "Please enter a thesis statement to improve.");
        }
        ...
        {
            ModelState.AddModelError(nameof(model.ThesisStatement), "Please enter a thesis statement or main argument.");
        }
    }
